...
public class PotionAchromatic extends PotionCustomTexture {
    ...
    public void performEffect(EntityLivingBase entity, int p_76394_2_) {
        /*
         * if(entity.boundingBox != null) { List<Entity> projectiles =
         * entity.worldObj.getEntitiesWithinAABB(IProjectile.class, entity.boundingBox.expand(3, 3, 3)); for(Entity e :
         * projectiles) { AxisAlignedBB boundingBox = e.boundingBox != null ? e.boundingBox :
         * AxisAlignedBB.getBoundingBox(e.posX, e.posY, e.posZ, e.posX, e.posY, e.posZ);
         * if(entity.boundingBox.expand(1.5, 1.5, 1.5).intersectsWith(boundingBox)) { double pMaxDist = new
         * Vector3(entity.boundingBox.minX, entity.boundingBox.minY, entity.boundingBox.minZ).distance(new
         * Vector3(entity.boundingBox.maxX, entity.boundingBox.maxY, entity.boundingBox.maxZ)); double eMaxDist = new
         * Vector3(boundingBox.minX, boundingBox.minY, boundingBox.minZ).distance(new Vector3(boundingBox.maxX,
         * boundingBox.maxY, boundingBox.maxZ)); double timeoutDist = pMaxDist + eMaxDist*2 + 4.5; Vector3 vec = new
         * Vector3(e.motionX, e.motionY, e.motionZ).normalize(); float dist = 0; boolean success = false; boolean state
         * = true; Vector3 temp; do { temp = vec.clone().multiply(dist); if(state) {
         * if(entity.boundingBox.intersectsWith(boundingBox.copy().offset(temp.getX(), temp.getY(), temp.getZ()))) {
         * state = false; } } else { if(!entity.boundingBox.intersectsWith(boundingBox.copy().offset(temp.getX(),
         * temp.getY(), temp.getZ()))) { success = true; break; } } dist += 0.1; } while(dist < timeoutDist); temp =
         * vec.clone().multiply(dist + 1.25); if(success) { e.posX += temp.getX(); e.posY += temp.getY(); e.posZ +=
         * temp.getZ(); } } } }
         */
