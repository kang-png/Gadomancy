...
public class ModConfig {
    ...
    private static void loadFromConfig() {
        ...
        ModConfig.entityNoClipItemDatawatcherMasterId = ModConfig.config.getInt(
                "itemNoClipDatawatcherMasterId",
                "entities",
                19,
                0,
                31,
                "Do not edit unless you know what are you doing! - Datawatcher ID of the Master-Coordinates for EntityNoClipItem");
        ModConfig.entityNoClipItemDatawatcherFixedId = ModConfig.config.getInt(
                "itemNoClipDatawatcherFixId",
                "entities",
                20,
                0,
                31,
                "Do not edit unless you know what are you doing! - Datawatcher ID of the Fixed-Coordinates for EntityNoClipItem");

        ModConfig.entityAuraCoreDatawatcherAspectsId = ModConfig.config.getInt(
                "auraCoreDatawatcherAuraId",
                "entities",
                21,
                0,
                31,
        ...
        ModConfig.dimOuterId = ModConfig.config.getInt(
                ...
                Integer.MIN_VALUE,
                Integer.MAX_VALUE,
                "Dimension Id for the eldrich mazes accessed via Node Manipulator");
        ModConfig.doLightCalculations = ModConfig.config.getBoolean(
                "calculateEldritchLight",
                "dimension",
                true,
                "TRUE = Calculating Light values for the Gadomancy-Eldritch Mazes; FALSE = No calculation, but some Light Bugs - Calculating the Light takes ~2 seconds -> Can be measured when trying to enter the eldritch mazed via Gadomancy Eldritch portal.");
        ModConfig.maxMazeCount = ModConfig.config.getInt(
                "maxMazeCount",
                "dimension",
                -1,
                -1,
                Integer.MAX_VALUE,
                "Defines how many Eldritch mazes may exist at the same time using the Gadomancy Eldritch ritual. (-1 = infinite) Note that 1 maze = 1 player; Once the player finishes the maze, the maze closes itself and teleports the player out.");

        // Skyblock stuff
        ModConfig.ancientStoneRecipes = ModConfig.config.getBoolean(
                "ancientStoneRecipes",
                "skyblock",
                false,
        ...
        ModConfig.renderParticleDistance = ModConfig.config.getInt(
                ...
                5,
                1000,
                "Defines, how close a player has to be towards the particle origin to see the particles created by it.");
        ModConfig.enableAdditionalNodeTypes = ModConfig.config.getBoolean(
                "enableAdditionalNodeTypes",
                "general",
                true,
                "Enables our custom node types. This might solve some compatibility issues (e.g. WitchingGadgets). You have to change this client- and server-side! Only change when you experience issues with special mods' features ONLY not working when using the mod together with Gadomancy.");
        String listOfAspects = ModConfig.config.getString(
                "auraPylonBlacklist",
                "general",
                "",
                "Write a list of aspects (e.g. aura,aer) here that should not be active/obtainable with the aura pylon. Multiple aspects can be seperated with ','. Leave it empty to blacklist nothing");
        ModConfig.blacklistAuraEffects = ModConfig.refactorAspects(listOfAspects);

        ModConfig.config.addCustomCategoryComment(
                "potions",
